...
document.addEventListener('DOMContentLoaded', function () {
  ...
  function fetchEvents(info, successCallback) {
    ...
    const ipAPI = BASEURL + '/json/getCalender'
let events
console.log('in');
    postData(ipAPI, {  })
        .then(response => {
            if (response && response.done) {
              ...
              return [response.data.events.filter(event => calendars.includes(event.extendedProps.calendar))];
    ...
    //  $.ajax(
    //   {
    //     url: 'http://127.0.0.1:8000/js/scripts/pages/app-calendar-events.js',
    //     type: 'GET',
  ...
  const updateEventInCalendar = (updatedEventData, propsToUpdate, extendedPropsToUpdate) => {
    ...
    for (let index = 0; index < propsToUpdate.length; index++) {
      let propName = propsToUpdate[index];
    ...
    for (let index = 0; index < extendedPropsToUpdate.length; index++) {
      let propName = extendedPropsToUpdate[index];
