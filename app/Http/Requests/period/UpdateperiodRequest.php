...
class UpdateperiodRequest extends GeneralRequest
{
    ...
    public function rules(Request $request)
    {
        ...
        return [
            'period_name' => 'required|string|' . Rule::unique('periods')->where('academic_year_id', $this->year)->ignore($this->period),
                        // 'period_name' => 'required|string|'. Rule::unique('periods')->where(function($query) use($request)  {
            //     $query->where('year_id','!=', $request->year_id)->where('periods.id','!=',$request->period->id);
            // }),
