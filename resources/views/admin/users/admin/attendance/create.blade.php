...
@php
$breadcrumbs = [[['link' => route('admins.index'), 'name' => "مديرين النظام"], ['link' => route('AttendanceManagers.index'), 'name' => "مشرفين الغياب"], [ 'name' => "اضافة مشرف"]],['title' => 'تسجيل مشرف']];
@endphp
...
<x-forms.formCard title="اضافة مشرف جديد">
    ...
    {!! Form::open(['route' => 'AttendanceManagers.store','method'=>'POST' , 'onsubmit' => 'showLoader()']) !!}
                ...
                <div class="col-6">
                    <select name="level_id[]" class="form-control{{ $errors->has('level_id') ? ' is-invalid' : null }}" id="level_id" multiple required style="min-height: 350px;">
                        ...
                        <optgroup label="{{ $type->school_name }} -> {{ $gender->gender_name }}">
                            ...
                            <option value="{{ $level->id }}">{{ $grade->grade_name }} -> {{ $level->level_name }}</option>
                            @endforeach
                            @endforeach
                            @endif
                            @endforeach
