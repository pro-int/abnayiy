...
@php
$breadcrumbs = [[['link' => route('coupons.index'), 'name' => "قسائم الخصم"],['link' => route('coupons.edit',$coupon), 'name' => "مشاهدة قسيمة $coupon->code" ]],['title'=> 'مشاهدة قسيمة']];
@endphp
...
@section('content')
@component('components.forms.formCard',['title' =>" مشاهدة قسيمة الخصم ($coupon->code)" ])
...
<div class="row mb-1">
        <div class="col-md">
            <x-inputs.select.generic style="pointer-events:none" select2="" label="العام الدراسي" name="academic_year_id" data-placeholder="اختر العام الدراسي" data-msg="رجاء اختيار العام الدراسي" :options="['' => 'اختر العام الدراسي'] + App\Models\AcademicYear::years()" onchange="get_CouponClassification()" />
        </div>
        ...
        <div class="col-md">
            ...
            <x-inputs.select.generic :required="false" style="pointer-events:none" select2="" label="نوع القسيمة" name="classification_id" data-placeholder="اختر نوع القسيمة" data-msg="رجاء اختيار نوع القسيمة" :options="['' => 'قسائم عامة'] + getCouponClassification($coupon->academic_year_id)" onchange="get_discount_values()" />
        </div>
    </div>

    <div class="row mb-1" id="selection">
        <div class="col-md">
            <x-inputs.select.generic style="pointer-events:none" select2="" label="المدرسة" name="school_id" data-placeholder="اختر المدرسة" data-msg="رجاء اختيار المدرسة" :options="['' => 'اختر المدرسة'] +schools()" />
        </div>
        ...
        <div class="col-md">
            <x-inputs.select.generic :required="false" style="pointer-events:none" select2="" label="الصف الدراسي" name="level_id" data-placeholder="اختر الصف الدراسي" data-msg="رجاء اختيار الصف الدراسي" :options="old('grade_id') ? App\Models\Level::levels(true,old('grade_id')) : App\Models\Level::levels(true,$coupon->grade_id)" onchange="get_discount_values()" />
        </div>
    </div>
    ...
    <x-ui.divider>معلومات القسيمة</x-ui-divider>
        ...
        <div class="row mb-1">
            <div class="col-md">
                <x-inputs.text.Input :disabled="true" class="only-english" onchange="toUpperCase(this)" label="رمز القسيمة" icon="file-text" name="code" placeholder="ادخل رمز القسيمة" data-msg="ارمز القسيمة بشكل صحيح" />
            </div>
            <div class="col-md">
                <x-inputs.text.Input label="قيمة الخصم" icon="file-text" name="coupon_value" placeholder="ادخل قيمة الخصم" data-msg="رجاء ادخال قيمة االخصم بشكل صحيح" />
            </div>
        </div>
        ...
        <script type="text/javascript">
            get_discount_values();
            toggleLevelSelection(document.getElementById('classification_id'))
        </script>
