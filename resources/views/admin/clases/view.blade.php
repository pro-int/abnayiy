...
@php
$breadcrumbs = [[['link' => route('home'), 'name' => "الرئيسية"],['link' => route('years.index'), 'name' => "السنوات الدراسية"],['link' => route('years.show',$year), 'name' => "$classroom->year_name"], ['link' => route('years.classrooms.index',$year), 'name' => 'الفصول الدراسية'],['name' => 'مشاهدة']],['title'=> 'مشاهدة فصل دراسي ']];
@endphp


@section('title', 'مشاهدة معلومات الفصل ')

@section('content')


@component('components.forms.formCard',['title' => 'مشاهدة معلومات الفصل '])
...
<x-ui.divider>معلومات الفصل</x-ui-divider>
    ...
    <x-ui.divider>معلومات المرحلة الخاصة بالفصل</x-ui-divider>
        ...
        <div class="row mb-1">
            ...
            <div class="col-md">
                <x-inputs.select.generic select2="" label="النوع" name="gender_id" data-placeholder="اختر النوع" data-msg="رجاء اختيار النوع" :options="$roles" :options="App\Models\Gender::genders(true,$classroom->school_id)" />
            </div>

            <div class="col-md">
                <x-inputs.select.generic select2="" label="المرحلة" name="grade_id" data-placeholder="اختر المرحلة" data-msg="رجاء اختيار المرحلة" :options="$roles" :options="App\Models\Grade::grades(true,$classroom->gender_id)" />
            </div>
