...
@php
$breadcrumbs = [[['link' => route('levels.index'), 'name' => "الصفوف الدراسية "],['link' => route('levels.edit',$level), 'name' => "تعديل الصف : $level->level_name" ]],['title'=> 'تعديل']];
@endphp

@section('title', 'تعديل الصف الدراسي ')

@section('content')
@component('components.forms.formCard',['title' =>' تعديل الصف الدرسي : ' . $level->level_name ])
...
<div class="row mb-1">
    ...
    <div class="row mb-1 ">
        ...
        <div class="col-md">
            <x-inputs.select.generic select2="" label="القسم" name="gender_id" data-placeholder="اختر القسم" data-msg="رجاء اختيار القسم" :options="old('school_id') ? App\Models\Gender::genders(true,old('school_id')) : App\Models\Gender::genders(true,$level->school_id)" />
        </div>

        <div class="col-md">
            <x-inputs.select.generic select2="" label="المسار" name="grade_id" data-placeholder="اختر المسار" data-msg="رجاء اختيار المسار" :options="old('gender_id') ?App\Models\Grade::grades(true,old('gender_id')) : App\Models\Grade::grades(true,$level->gender_id)" />
        </div>
    ...
    <div class="row mb-1">
        <div class="col-md">
            <x-inputs.text.Input type="number" label="الحد الادني لعدد الطلاب لأفتتاح فصل" icon="user-plus" name="min_students" placeholder="الحد الادني لعددالطلاب في قائمة الانتظار لتسجيل فصل جديد" data-msg="ادخل الحد الأدني لعدد الطاب" />
        </div>
    ...
    <div class="col-12 text-center mt-2">
        <x-inputs.submit>تعديل المسار </x-inputs.submit>
