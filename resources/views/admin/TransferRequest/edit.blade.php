...
@section('title', sprintf('تعديل الطلب رقم : %s | %s',$transfer->id , $transfer->student_name))
            ...
            <x-ui.divider>خطة دفع الرسوم الدراسية</x-ui-divider>
                ...
                <x-ui.divider>خطة النقل</x-ui-divider>
                    ...
                    <div class="row mb-1">
                        ...
                        @if($transfer->req_confirmation)
                        <x-ui.divider>خطة السداد (<span class="text-danger">{{ $transfer->plan_name }}</span>) تتطلب سند امر</x-ui-divider>

                            <div class="row mb-1">
                                <div class="col-md">
                                    @if (in_array($transfer->status,['new','pending']))
                                    ...
                                    @elseif(in_array($transfer->status,['complete']))
                            ...
                            <x-ui.divider color="success">تفاصيل السداد</x-ui-divider>
                                @php $hasFile = $transfer->payment_ref && Storage::disk('s3')->exists($transfer->payment_ref) @endphp

                                @if (in_array($transfer->status,['new','pending']))
                                <div class="row mb-1">
                                    <div class="col-md">
                                        <x-inputs.select.generic select2="" name="method_id" label="طريقة الدفع" onchange="checkPaymentMethod()" :options="App\Models\PaymentMethod::paymentMethods(1,[1,2])" />
                                    </div>

                                    <div class="col-md">
                                        <x-inputs.select.generic name="bank_id" label="البنك" :options="['' => 'اختر البنك'] + GetBanks()" />
                                    </div>
                                </div>
                                ...
                                <div class="row mb-1">
                                    <div class="col-md">
                                        ...
                                        <x-inputs.checkbox required name="confirmed">{!! sprintf(' قام ولي الأمر بسداد مبلغ <span class="text-danger">%s</span> ر.س ( <span class="text-danger">%s</span> رسوم دراسية - <span class="text-danger">%s</span> رسوم النقل - <span class="text-danger">%s</span> مديونيات سابقة)', $transfer->minimum_tuition_fee + $transfer->bus_fees + $transfer->bus_fees_vat + $transfer->dept_paid, $transfer->minimum_tuition_fee , $transfer->bus_fees + $transfer->bus_fees_vat , $transfer->dept_paid) !!} </x-inputs.checkbox>
                                    </div>
                                </div>
                    </div>
                    @else
                    ...
                    <div class="col-12 text-center mt-2">
                        @if (in_array($transfer->status,['new','pending']))
                        <x-inputs.submit>تأكيد استلام دفعة التعاقد وتسجيل الطالب</x-inputs.submit>
                        @endif
