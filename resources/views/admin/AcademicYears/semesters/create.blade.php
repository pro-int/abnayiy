...
@php
$breadcrumbs = [[['link' => route('home'), 'name' => "الرئيسية"],['link' => route('years.index'), 'name' => "السنوات الدراسية"],['link' => route('years.show',$year->id), 'name' => $year->year_name],['link' => route('years.semesters.index',$year->id), 'name' => 'الفصول الدراسية'],['name'=> 'تسجيل فصل دراسي', 'link' => route('years.semesters.create',$year->id)] ],['title' => 'تسجيل فصل دراسي']];
@endphp

@section('title', 'تسجيل فصل دراسي')
...
@component('components.forms.formCard',['title' => 'تسجيل فصل دراسي جديد'])

{!! Form::open(['route' => ['years.semesters.store','year'=> request()->year],'method'=>'POST' , 'onsubmit' => 'showLoader()'])  !!}
...
<x-ui.divider>معلومات الفصل الدراسي</x-ui-divider>
    ...
    <div class="col-12 text-center mt-2">
        <x-inputs.submit >اضافة الفصل الدراسي</x-inputs.submit>
